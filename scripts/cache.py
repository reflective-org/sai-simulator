...
from backend.baseline import get_global_temp, get_regional_models, get_regional_map_from_global_temp, get_regional_map
from backend.delta import get_regional_delta
from backend.historical import get_historical_model
from backend.p_values import get_grid_level_p_values
...
from backend.icefrac import predict_icefrac

def cache_historical(data_dir, output_dir):
    """Pre-generate shared historical_*.nc files (safe to call before parallel runs)."""
    data_dir = Path(data_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(exist_ok=True)

    print("Pre-generating historical cache files...")
    for var in VAR2INFO:
        historical_model = get_historical_model(var, data_dir, output_dir)
        if historical_model is not None:
            print(f"  historical_{var}.nc ready")
    print("Historical cache complete.")


def cache(data_dir, model_dir, output_dir, scenario=None, historical_only=False):
    """Generate cache files for all (or a single) SSP scenario.

    Args:
        data_dir: Path to processed data directory
        model_dir: Path to fitted models directory
        output_dir: Path to cache output directory
        scenario: Optional SSP scenario name (e.g. 'SSP1-1.9') to cache only that one.
                  If None, caches all scenarios.
        historical_only: If True, only generate shared historical_*.nc files and exit.
                         Use this before running per-scenario caching in parallel.
    """
    data_dir = Path(data_dir)
    ...
    if historical_only:
        cache_historical(data_dir, output_dir)
        return

    if scenario is not None:
        if scenario not in REVERSE_FANCY_SSP_TITLES:
            print(f"Error: Unknown scenario '{scenario}'")
            print(f"Valid scenarios: {list(REVERSE_FANCY_SSP_TITLES.keys())}")
            sys.exit(1)
        scenarios = [scenario]
    else:
        scenarios = list(REVERSE_FANCY_SSP_TITLES.keys())
    ...
    for ssp_scenario in scenarios:
        ...
        for var in tqdm(VAR2INFO):
            ...
            # cache no sai regional map and p values
            ...
            if all(cache_path.exists() for cache_path in no_sai_cache_paths):
                ...
                for cache_path in no_sai_cache_paths:
                    print(cache_path)
                with xr.open_dataarray(no_sai_cache_paths[0]) as da:
                    regional_map = da.load()
                with xr.open_dataarray(no_sai_cache_paths[1]) as da:
                    regional_p_values = da.load()
            else:
                is_exposure = "exposure" in var

                if var == "icefrac":
                    regional_temp, regional_temp_no_sai_p_values = get_regional_map(
                        "tas", data_dir, model_dir, output_dir, ssp_scenario
                    )
                    regional_map = predict_icefrac(regional_temp, model_dir)
                    regional_p_values = get_grid_level_p_values(
                    ...
                    regional_map, regional_p_values = get_regional_map_from_global_temp(
                        global_temp, fair2smip, var, data_dir, output_dir
                    )
            ...
            for temp_target in np.linspace(MIN_TARGET, MAX_TARGET, num_targets):
                ...
                if not so2_cache_path.exists():
                    ...
                    so2_by_latitude = xr.concat(so2_by_latitudes, dim='start_year', join='outer')
                ...
                if var == "icefrac":
                    temp_regional_delta = get_regional_delta(
                        "tas", data_dir, model_dir, output_dir, ssp_scenario, 
                        temp_target, 10, temp_diff, start_year
                    )
                    ...
                    # Save the p_values
                    regional_sai_p_values = get_grid_level_p_values(var, data_dir, historical_rebase, regional_map + regional_delta + historical_rebase)
                
                regional_delta.to_netcdf(sai_cache_paths[0])
